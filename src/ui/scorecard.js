...
export function renderPlayCourseBar(courseId, callbacks = {}) {
  ...
  function updateBar() {
    ...
    summaryBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 9L20 9M8 9V20M6.2 20H17.8C18.9201 20 19.4802 20 19.908 19.782C20.2843 19.5903 20.5903 19.2843 20.782 18.908C21 18.4802 21 17.9201 21 16.8V7.2C21 6.0799 21 5.51984 20.782 5.09202C20.5903 4.71569 20.2843 4.40973 19.908 4.21799C19.4802 4 18.9201 4 17.8 4H6.2C5.0799 4 4.51984 4 4.09202 4.21799C3.71569 4.40973 3.40973 4.71569 3.21799 5.09202C3 5.51984 3 6.07989 3 7.2V16.8C3 17.9201 3 18.4802 3.21799 18.908C3.40973 19.2843 3.71569 19.5903 4.09202 19.782C4.51984 20 5.07989 20 6.2 20Z" stroke="currentColor" stroke-width="1.68" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  function renderSummary() {
    ...
    let totalStrokes = 0, totalPar = 0, totalPutts = 0, totalGIR = 0, holesPlayed = 0;
    played.forEach(h => {
      if (h.total != null) {
    ...
    let front9Par = 0, front9Strokes = 0, front9Putts = 0, front9GIR = 0, front9Played = 0;
    played.slice(0, 9).forEach(h => {
      front9Par += h.par;
      if (h.total != null) { front9Strokes += h.total; front9Putts += h.putts; if (h.gir === true) front9GIR++; front9Played++; }
    });

    let back9Par = 0, back9Strokes = 0, back9Putts = 0, back9GIR = 0, back9Played = 0;
    played.slice(9, 18).forEach(h => {
      back9Par += h.par;
      if (h.total != null) { back9Strokes += h.total; back9Putts += h.putts; if (h.gir === true) back9GIR++; back9Played++; }
    });
